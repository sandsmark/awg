...
public class Map {
	...
	public void selectUnit(int x, int y) {
		...
		// + y2);
		for (int i = 0; i < getUnitNum(); i++) {
			// System.out.println("unit :: x:" + getUnit(i).getX() + " y:" +
			// getUnit(i).getY());
			if ((y1 < getUnit(i).getPosition().y)
					&& (y2 > getUnit(i).getPosition().y)
					&& (x1 < getUnit(i).getPosition().x)
					&& (x2 > getUnit(i).getPosition().x)) {
	...
	public void moveSelectedTo(int tarX, int tarY) {
		...
		for (int i = 0; i < getSelectedUnitNum(); i++) {
			path = new Path(this, getSelectedUnit(i).getPosition().x,
					getSelectedUnit(i).getPosition().y, tarX, tarY);
